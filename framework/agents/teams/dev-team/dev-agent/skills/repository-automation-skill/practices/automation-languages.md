...
This is a P1 hard rule. A violation blocks acceptance.

## Problem Pattern

Python adds another repository language and runtime for tasks already covered by
...
Validate repository language usage, infrastructure manifests, formatting, and
behavior through the project's corresponding checks.

The repository-language check must use one ignore-aware filesystem inventory in
...
The scan must reject prohibited source, package, notebook, and executable-artifact
...
Taskfile declarations remain declarative tasks rather than authored functions.
