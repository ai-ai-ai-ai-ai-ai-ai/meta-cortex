...
### Enforce the Option prohibition with Clippy

- Add the following entry to `disallowed-types` in each applicable `clippy.toml`.
- Preserve existing disallowed-type entries and other Clippy settings.
- Deny `clippy::disallowed_types` for every applicable crate.
  - For shared workspace lints, ensure each member inherits the workspace lint settings.
- Run Clippy for all targets with warnings denied.
- Check compatibility with required derives before declaring the gate operational.
  - Clippy can report `Option` uses generated by Serde and thiserror derives.
  - Isolate affected declarations in a small module containing only types and derives.
  - Allow this lint on that module with a reason naming the generating dependency.
  - Deny the lint explicitly on every authored type in the module.
  - Keep authored behavior outside the module under the normal lint policy.
  - Verify an authored `Option` field still fails before accepting the exception.
  - Do not disable the lint across the crate or replace required derives to evade it.
- Review inferred stored values and dependency-result handling separately.
  - This lint does not establish compliance with the whole no-`Option` rule.

**Prohibited:** rely on the prose prohibition without configuring the lint.

**Preferred — `clippy.toml`:**

```toml
disallowed-types = [
    { path = "core::option::Option", reason = "Option is prohibited in this project" },
]
```

**Preferred — crate `Cargo.toml`:** merge this setting into the existing lint table.
...
**Prohibited:** disable `disallowed_types` for the whole crate because a derive fails.

**Preferred:** scope the allowance to generated sibling implementations and
restore denial on the authored declaration. This fragment assumes an existing
`CacheState` type implementing `Deserialize` in the parent module.

```rust
#[allow(clippy::disallowed_types, reason = "Serde generates Option internally")]
mod cache {
...
The earlier `OrderCache` examples demonstrate the prohibited type and its named
replacement. Keep immediate matching of dependency-returned values at the boundary.
