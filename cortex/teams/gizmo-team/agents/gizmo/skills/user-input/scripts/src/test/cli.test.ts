...
class CliScenario {
  static readonly modeSchema = new URL(
    "../../../../../../../../../development.yaml",
    import.meta.url,
  ).pathname;
...
test("CLI emits the shipped development-mode native question", () => {
  const input: CliInput = {
    arguments: [`--schema=${CliScenario.modeSchema}`],
    request:
...
test.each(["single_agent", "multi_agent"])(
  "CLI accepts development mode %s",
  (mode) => {
    const input: CliInput = {
      arguments: [`--schema=${CliScenario.modeSchema}`],
    ...
    const result = new CliScenario(input).execute();
    expect(result.exitCode).toBe(0);
    expect(result.stdout.toString()).toContain('"status":"complete"');
